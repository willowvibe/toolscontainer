...
class VideoConverterGUI(ctk.CTk):
    def __init__(self):
        ...
        self.select_input_folder_button = ctk.CTkButton(self.input_folder_frame, text="Browse", command=self.select_input_folder)
        ...
        self.select_output_folder_button = ctk.CTkButton(self.output_folder_frame, text="Browse", command=self.select_output_folder)
    ...
    def update_progress(self, file, status, current_index=0, total_files=1, time_taken=None):
        def update():
            ...
            # Update time taken label if conversion is completed
            if time_taken is not None:
                self.time_taken_label.configure(text=f"Time: {time_taken}s")
